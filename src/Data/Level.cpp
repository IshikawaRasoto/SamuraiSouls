...
Characters::Enemies::Boss* Level::buildBoss(Snapshots::EntitySnapshot* entitySnapshot){
    Characters::Enemies::Boss* boss = new Characters::Enemies::Boss(
        entitySnapshot->getPosition(),
        new Entities::FireBall(entitySnapshot->getPosition())
    );
...
Lists::EntityList* Level::loadSnapshots(Patterns::StateId levelState){
    ...
    while(std::getline(file, line)){
        ...
        switch(entityType){
            ...
            case Type::Boss:
                list->addEntity(buildBoss(entitySnapshot));
                break;
            case Type::Goblin:
