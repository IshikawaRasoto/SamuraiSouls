    ...
    Level(stateMachine, SECOND_LEVEL_BACKGROUND_DIR, Patterns::StateId::SecondLevel, singlePlayer)
{
    ...
    for(i = -3; i < 6; i++){
        Entities::Objects::Surfaces::Ground *ground = new Entities::Objects::Surfaces::Ground({float(i)*GROUND_WIDTH, 10});
    ...
    for(i; i < 9; i++){
        Entities::Objects::Surfaces::Ground *ground = new Entities::Objects::Surfaces::Ground({float(i)*GROUND_WIDTH + float(j) * THORNS_WIDTH, 10});
        staticEntities->addEntity(ground);
        entityList.addEntity(ground);
    }

    for(i; i < 15; i++){
        Entities::Objects::Surfaces::Ground *ground = new Entities::Objects::Surfaces::Ground({float(i)*GROUND_WIDTH + float(j) * THORNS_WIDTH, 0});
        staticEntities->addEntity(ground);
        entityList.addEntity(ground);
    }
